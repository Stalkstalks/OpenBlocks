...
public class CommandInventory implements ICommand {
    ...
    public String getCommandUsage(ICommandSender icommandsender) {
        return NAME + " store <player> OR "
                ...
                + NAME
                + " spawn <file without 'inventory-' and '.dat'> [<sub_inventory OR '"
                + ID_MAIN_INVENTORY
                + "'>,  [<index of item>]]";
    }
        ...
        if (subCommand.equalsIgnoreCase(COMMAND_RESTORE)) {
            ...
            try {
                ...
                sender.addChatMessage(
                        new ChatComponentTranslation("openblocks.misc.stored_inventory", result.getAbsolutePath()));
            } catch (Exception e) {
            ...
            for (ItemStack stack : toRestore) if (stack != null)
                BlockUtils.dropItemStackInWorld(sender.getEntityWorld(), coords.posX, coords.posY, coords.posZ, stack);
    ...
    public List addTabCompletionOptions(ICommandSender sender, String[] args) {
        ...
        if (subCommand.equals(COMMAND_SPAWN)) {
            if (args.length == 2)
                return PlayerInventoryStore.instance.getMatchedDumps(sender.getEntityWorld(), args[1]);

            if (args.length == 3) {
                final String fileId = args[1];
                try {
                    LoadedInventories inventories = PlayerInventoryStore.instance
                            .loadInventories(sender.getEntityWorld(), fileId);
            ...
            if (args.length == 3) {
                ...
                if (subCommand.equalsIgnoreCase(COMMAND_RESTORE))
                    return PlayerInventoryStore.instance.getMatchedDumps(sender.getEntityWorld(), prefix);
            }
        }
