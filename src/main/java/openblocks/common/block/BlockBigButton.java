...
public class BlockBigButton extends OpenBlock {
    ...
    public boolean canPlaceBlock(World world, EntityPlayer player, ItemStack stack, int x, int y, int z,
            ForgeDirection sideDir, Orientation blockDirection, float hitX, float hitY, float hitZ, int newMeta) {
        return super.canPlaceBlock(world, player, stack, x, y, z, sideDir, blockDirection, hitX, hitY, hitZ, newMeta)
                && isNeighborBlockSolid(world, x, y, z, blockDirection.north());
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        ...
        final AxisAlignedBB aabb = AxisAlignedBB
                .getBoundingBox(0.0625, 0.0625, 0, 0.9375, 0.9375, pressed ? 0.0625 : 0.125);
