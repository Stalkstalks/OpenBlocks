        ...
        public void run() {
            ...
            try {
                while (running) {
                    ...
                    if (available >= SAMPLES_PER_BUFFER) writeSample(line);
    ...
    private byte[] generateSamplesWithSweep(double f0, double f1) {
        ...
        if (samplesPerBeep == 0) {
            ...
            for (int i = 0; i < BYTES_PER_BUFFER; i += BYTES_PER_SAMPLE, sampleCount++) {
                ...
                if (beepSample++ >= 2 * samplesPerBeep) beepSample = 0;
            }
