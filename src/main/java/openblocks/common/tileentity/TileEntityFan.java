...
public class TileEntityFan extends SyncedTileEntity implements IPlacerAwareTile, INeighbourAwareTile, IAddAwareTile {
    ...
    public void updateEntity() {
        ...
        final Vec3 coneAxis = Vec3.createVectorHelper(
                basePos.xCoord - blockPos.xCoord,
                basePos.yCoord - blockPos.yCoord,
                basePos.zCoord - blockPos.zCoord);
    ...
    private Vec3 getConeApex(double angle) {
        return Vec3.createVectorHelper(
                xCoord + 0.5 - Math.cos(angle) * 1.1,
                yCoord + 0.5,
                zCoord + 0.5 - Math.sin(angle) * 1.1);
    }

    private AxisAlignedBB getEntitySearchBoundingBox() {
        AxisAlignedBB boundingBox = AxisAlignedBB
                .getBoundingBox(xCoord, yCoord - 2, zCoord, xCoord + 1, yCoord + 3, zCoord + 1);
