...
public class TileEntityBuilderGuide extends TileEntityGuide implements IGuideAnimationTrigger {
    ...
    private void creativeReplaceBlocks(Block block, int blockMeta) {
        for (Coord coord : getShapeSafe().getCoords()) worldObj
                .setBlock(xCoord + coord.x, yCoord + coord.y, zCoord + coord.z, block, blockMeta, BlockNotifyFlags.ALL);
    }
    ...
    private boolean survivalPlaceBlocks(ItemBlock itemBlock, EntityPlayerMP player, ItemStack heldItem, Block block,
            int blockMeta, int side, float hitX, float hitY, float hitZ) {
        for (Coord relCoord : getShapeSafe().getCoords()) {
            ...
            if (worldObj.blockExists(absX, absY, absZ) && worldObj.isAirBlock(absX, absY, absZ)) {
                ...
                BlockEvent.PlaceEvent event = new BlockEvent.PlaceEvent(
                        snapshot,
                        worldObj.getBlock(absX, absY, absZ),
                        player);
