...
public class TileEntityTank extends SyncedTileEntity
    ...
    private void tryBalanceNeighbors(FluidStack contents) {
        ...
        for (TileEntityTank n : neighbors) sum += n.tank.getFluidAmount();

        final int suggestedAmount = sum / (count + 1);
        if (Math.abs(suggestedAmount - contents.amount) < Config.tankFluidUpdateThreshold) return; // Don't balance
                                                                                                   // small amounts to
                                                                                                   // reduce server load
