...
public class MagnetControlAdapter implements ITickingTurtle, IWorldProvider, IAttachable {
    ...
    public class Owner implements IOwner {
        ...
        public EntityBlock createByPlayer(final IEntityBlockFactory factory) {
            ...
            if (world instanceof WorldServer) return FakePlayerPool.instance
                    .executeOnPlayer((WorldServer) world, new PlayerUserReturning<EntityBlock>() {
    ...
    @ScriptCallable(
            returnTypes = { ReturnType.NUMBER, ReturnType.NUMBER, ReturnType.NUMBER },
    ...
    public IMultiReturn getDistanceToTarget() {
        ...
        return MultiReturn
                .wrap(current.xCoord - target.xCoord, current.yCoord - target.yCoord, current.zCoord - target.zCoord);
    }
    ...
    private static boolean checkTargetRange(double x, double y, double z) {
        return Math.abs(x) <= Config.turtleMagnetRange && Math.abs(y) <= Config.turtleMagnetRange
    ...
    private boolean canSpawn(World world) {
        ...
        ForgeDirection spawnSide = facing
                .getRotation((side == SpawnSide.Left) ? ForgeDirection.DOWN : ForgeDirection.UP);
