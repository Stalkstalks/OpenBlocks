...
public class MagnetTurtleUpgrade implements ITurtleUpgrade {
    ...
    public IPeripheral createPeripheral(ITurtleAccess turtle, TurtleSide side) {
        return ApiAccess.getApi(IComputerCraftObjectsFactory.class)
                .createPeripheral(new MagnetControlAdapter(turtle, side));
    }
    ...
    public void registerIcons(TextureStitchEvent evt) {
        if (evt.map.getTextureType() == TextureUtils.TEXTURE_MAP_BLOCKS)
            icon = evt.map.registerIcon("openblocks:magnet_upgrade");
    }
