...
public class BlockProjectorRenderer implements IBlockRenderer<BlockProjector> {
    ...
    public void renderInventoryBlock(BlockProjector block, int metadata, int modelID, RenderBlocks renderer) {
        ...
        if (!depthMaskState) GL11.glDepthMask(true);
        ...
        if (block.shouldRenderTesrInInventory()) {
            ...
            if (renderTe instanceof OpenTileEntity)
                ((OpenTileEntity) renderTe).prepareForInventoryRender(block, metadata);
            TileEntityRendererDispatcher.instance.renderTileEntityAt(renderTe, -0.5, -0.5, -0.5, 0.0F);
        }

        if (!depthMaskState) GL11.glDepthMask(false);
    }

    @Override
    public boolean renderWorldBlock(final IBlockAccess world, int x, int y, int z, BlockProjector block, int modelId,
