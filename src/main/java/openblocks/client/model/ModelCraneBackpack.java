...
public class ModelCraneBackpack extends ModelBiped {
    ...
    @Override
    public void setRotationAngles(float swingTime, float swingAmpl, float rightArmAngle, float headAngleX,
            float headAngleY, float scale, Entity entity) {
    ...
    public void renderLines(RenderPlayerEvent.Pre evt) {
        ...
        final float offset = interpolateAngle(
                player.renderYawOffset,
                player.prevRenderYawOffset,
                evt.partialRenderTick);
    ...
    private static void drawLineFPP(EntityPlayer player, float partialTickTime) {
        ...
        final double magnetY = interpolatePos(magnet.posY, magnet.lastTickPosY, partialTickTime) - centerY
                + magnet.height
                - 0.05;
